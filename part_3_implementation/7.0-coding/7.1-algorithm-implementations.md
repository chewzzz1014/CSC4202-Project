...
{% code title="DijkstraAlgorithm.java" fullWidth="false" %}
```java
...
class DijkstrasAlgorithm {
	...
	static void dijkstra(double[][] graph, int src, int dest) {
		...
		for (int count = 0; count < V; count++) {
			...
			if (u == -1)
				return; // All remaining nodes are unreachable, exit the loop
